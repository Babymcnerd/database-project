...
conn = psycopg2.connect(host="localhost", dbname="disks-db",
...
def init():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS bag (
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS discsinbag (
        b1 VARCHAR(255),
        b2 VARCHAR(255),
        b3 VARCHAR(255),
        b4 VARCHAR(255),
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS proplayer (
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS disc (
        speed INT,
        glide INT,
        turn INT,
        fade INT,
    ...
    # cur.execute("""INSERT INTO disc (speed, glide, turn, fade, material, name, discid) VALUES
    ...
    # cur.execute("""INSERT INTO proplayer (playerid, ranking, name, bagid, companyname) VALUES
    ...
    # cur.execute("""INSERT INTO bag (bagid, numberofdiscs, bagname, owner) VALUES
    ...
    # cur.execute("""INSERT INTO discsinbag (b1, b2, b3, b4, b5, b6, b7) VALUES
    ...
    # cur.execute("""INSERT INTO person (name, username, password) VALUES
    ...
    # cur.execute("""INSERT INTO company (name, hqlocation) VALUES
...
def HandleUserInput():
    ...
    if command in commands:
        ...
        print (f"command {command} is not a command please try again or use the command \"help\" for a list of command you can use")

def printUserBag():
    if not loggedIn:
        print("You must be logged in to use this command. Please login with 'login username password'.")
        return
...
def login(usernameinput, passwordinput) -> bool:
    ...
    if not isinstance(checkinputreturn, NoneType):
        ...
        print(f"User {usernameinput} is not a user to signup use the command: signup username password")
...
def showDiscs():
    ...
    cur.execute(query)
    response = cur.fetchall()
    print(f"these are the disks currently in the database that can be added to your bag: \n{response}")
...
def showPlayer(proName):
    ...
    bag = cur.fetchone()
    if bag[0] == None:
        print(f"either pro {proName} is not in the database or does not have a registered bag")
    query = f"SELECT {bag[0]} FROM discsinbag"
    cur.execute(query)
    response = cur.fetchall()
    # print(response[0][0])
...
# def showPlayer(proplayername):
#     checkinput = f"SELECT name FROM proplayer WHERE name='{proplayername}';"
#     cur.execute(checkinput)
#     proname = cur.fetchone()
#     query1 = f"SELECT bagid FROM proplayer WHERE name='{proname[0]}';"
#     cur.execute(query1)
#     bagid = cur.fetchall()
#     # query2 = f"SELECT * FROM discsinbag WHERE bagid='{bagid}';"
#     i = 0
#     query2 = f"SELECT '{bagid[0][0]}' FROM discsinbag;"
#     cur.execute(query2)
#     print(cur.fetchone())
#     # for i, disc in enumerate(discId):
#     #     if disc != "NULL":
#     #         print(discId[i])
